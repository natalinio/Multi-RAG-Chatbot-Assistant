...
/**
 * Helper to decode HTML entities (Fixes the &gt; issue in Mermaid)
 */
        ...
        if (window.mermaid && container.querySelectorAll('.mermaid').length > 0) {
            ...
            setTimeout(() => {
                ...
                mermaidElements.forEach((element, index) => {
                    ...
                    // IMPORTANT: Decode HTML entities inside the element content before rendering
                    // This fixes issues where --> becomes &gt;
                    const rawContent = element.innerHTML;
                    const decodedContent = decodeHtmlEntities(rawContent);
                    if (rawContent !== decodedContent) {
                ...
                window.mermaid.run({
                    ...
                    mermaidElements.forEach(el => {
                        if (!el.getAttribute('data-processed')) {
                            el.innerHTML = '<div style="color: #ff6b6b; padding: 20px; border: 1px solid #ff6b6b; border-radius: 8px;">Failed to render diagram. Syntax error in graph definition.</div>';
                        }
                    });
                });
            }, 100);
...
function formatBotMessage(message) {
    ...
    let html = message.replace(/```(\w*)\s*([\s\S]*?)```/g, function(match, language, code) {
        const lang = (language || 'json').trim();
        let cleanCode = code.trim();
        
        // FIX: If it is a mermaid diagram, make sure we decode entities 
        // (in case backend sent &gt; for > inside the block)
            ...
            const formattedBlock = `<div class="mermaid-block" id="${blockId}">
                ...
                <div class="mermaid-content">
                    <div class="mermaid-wrapper" id="${blockId}-wrapper">
                        <!-- We insert raw code here, decodeHtmlEntities will be called before rendering -->
                        <div class="mermaid" data-mermaid-code="${escapeHtml(code)}">${code}</div>
                    </div>
                </div>
            </div>`;
